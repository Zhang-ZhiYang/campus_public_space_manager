...
# --------------------------------------------------------------------
# 用于运行时安全导入及模拟其他应用的模型
# Mock Objects 不再需要 for_user，直接返回空的 QuerySet 即可
# --------------------------------------------------------------------
...
class BookableAmenityInline(admin.TabularInline):
    ...
    def save_model(self, request, obj, form, change):
        ...
        if not (request.user.is_superuser or request.user.is_system_admin or \
...
class SpaceAdmin(GuardedModelAdmin):
    ...
    fieldsets = (
        ...
        ('预订组限制', {'fields': ('restricted_groups',),
    ...
    def get_queryset(self, request):
        qs = super().get_queryset(request)  # This will be the base QuerySet `Space.objects.all()`
        if not request.user.is_authenticated: return qs.none()
        if request.user.is_superuser or request.user.is_system_admin: return qs
    ...
    def get_actions(self, request):
        ...
        if request.user.is_space_manager and not request.user.is_system_admin:
    ...
    def make_spaces_bookable(self, request, queryset):
        ...
        if not (request.user.is_superuser or request.user.is_system_admin):
            # object-level permission check for each object in the queryset
    ...
    def safer_delete_selected(self, request, queryset):
        ...
        if not BOOKINGS_MODELS_LOADED:
            ...
            for space in queryset:
                if request.user.has_perm('spaces.delete_space', space):  # has_perm still works for individual objects
    ...
    def has_view_permission(self, request, obj=None):
        ...
        if request.user.is_superuser or request.user.is_system_admin: return True
        if obj is None:
            return request.user.is_space_manager
